...
import { useParams } from 'react-router-dom'; // Import useParams to handle URL parameters
...
import trashBinIcon from "../image/trash-bin.png";

const ProfilePostModal = ({ show, handleClose, post }) => {
  ...
  useEffect(() => {
    if (post) {
      ...
      // Fetch user data by userId from URL parameters
      if (userId) {
        fetch(`http://localhost:5000/user/user/${userId}`)
          .then(response => response.json())
    ...
    <>
      <Modal show={show} onHide={handleClose} centered size="lg">
        <Modal.Header closeButton>
          <Modal.Title>Post by {user?.username || "Unknown User"}</Modal.Title>
        </Modal.Header>
